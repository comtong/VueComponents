<template>
  <div class="vpl-view">
    ...
    <div
      ...
      :key="i + 2"
      :class="vplViewItemClass(i + 2, key)"
      @click="selectKey(i + 2)"
    >
      <div
...
export default {
  ...
  methods: {
    ...
    vplViewItemClass(i, key) {
      let list = ["vpl-key"];
      if (this.sindex === i) {
        list.push("border-select");
      } else {
        if (this.smax - 1 === i && key === "") {
          list.push("border-newpower");
        } else {
          list.push("border-unselect");
        }
      }
      return list;
    }
